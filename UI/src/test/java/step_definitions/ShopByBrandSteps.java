...
////                Thread.sleep(1000); // Allow time for the scrolling effect
//
//
...
//        }
        ...
        for (WebElement item : brandItems) {
            ...
            try {
                // Scroll into view and adjust for sticky headers
                ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView({block: 'center', inline: 'nearest'});", link);
                Thread.sleep(500);
                ...
                if (!isClicked) {
                    throw new RuntimeException("Failed to click on the element after retries: " + brandName);
                }
